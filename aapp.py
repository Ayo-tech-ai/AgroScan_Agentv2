...
# Everything inside this block should only ever run ONCE per
# browser session — on first load — not on every message.
# ============================================================
...
# Defined at module level (cheap, no side effects until called).
# ============================================================
...
# ============================================================
# RUN DATABASE SETUP
# create_farm_records_table() is idempotent (CREATE TABLE IF NOT
# EXISTS), safe to call every time the container cold-starts.
#
# The Excel import only runs if the table is genuinely empty —
# this matters because the SQLite file persists at the CONTAINER
# level across every visitor's browser session, not just yours.
# Without this check, every new visitor opening the app would
# re-attempt importing all 365+ historical rows again (harmless,
# since duplicates get skipped via IntegrityError, but wasteful
# and slow for every single new visitor).
# ============================================================

create_farm_records_table()

if get_record_count() == 0:
    imported, skipped = initialize_farm_record_book(EXCEL_FILE)
    st.session_state.import_summary = f"Imported {imported} historical records."
else:
...
# is skipped entirely, and the already-built Runner/session is
# pulled from st.session_state instead.
# ============================================================
...
if not st.session_state.initialized:
    ...
    # Identity, tone, and communication style. Unchanged from
    # the notebook.
    # --------------------------------------------------------
...
"""
    ...
    # Final version from today's session: crates_collected rule
    # removed, ₦ currency instruction added, lookup/summary
    # guidance included.
    # --------------------------------------------------------
...
# --------------------------------------------------------
    # FarmRecordService
    # Unchanged from the notebook — the inheritance fix
    # (today's-own-record checked before previous day) and the
    # crates_collected-is-now-optional change are both included.
    # --------------------------------------------------------
    ...
    # Groq (our primary model) sometimes sends numeric tool
    # arguments as strings (e.g. "30" instead of 30). These
    # helpers tolerate that, converting safely, and cleaning
    # common formatting like currency symbols or thousands
    # separators a model or farmer might include.
    # --------------------------------------------------------
    ...
    # crates_collected is now OPTIONAL (per today's decision to
    # remove the always-required constraint).
    # --------------------------------------------------------
    ...
    # One SkillToolset per Agent, both skills together — the
    # rule we confirmed after the multi-toolset collision issue.
    # additional_tools left empty — all four tools are registered
    # directly on the Agent instead (Option B: always available,
    ...
    # --------------------------------------------------------
    # AGENT
    # Model: Groq via LiteLlm, per today's decision. API key is
    # read from Streamlit's secrets manager, not Colab userdata.
    # today_str is computed fresh on this first-ever build for
    # the session, so the agent starts with an accurate date.
    # --------------------------------------------------------
    ...
    # This must run BEFORE the Agent/LiteLlm object is created.
    # --------------------------------------------------------
    import os
...
""",
    ...
    # session (via uuid), not hardcoded — this matters if
    # multiple people use the deployed app at the same time.
    # --------------------------------------------------------
    ...
    # From this point on, on every future rerun of the script
    # (i.e. every message sent), this whole block is skipped,
    # and these objects are simply retrieved from session_state.
    # --------------------------------------------------------
    ...
    st.session_state.chat_history = []  # for rendering messages in the UI

    st.session_state.initialized = True

    # ============================================================
# ASYNC BRIDGE
# runner.run_debug(...) is an async function. Streamlit's script
# executes synchronously, so we need a small helper to run it
# and get the result back, rather than calling it directly.
# ============================================================
...
# On every rerun, redraw every message that's already happened
# in this session, before handling any new input.
# ============================================================
...
if user_message:
    ...
    # Run the agent and show its response
